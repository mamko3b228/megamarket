...
async function buyGoodOnMegaMarket(goodId, merchant) {
    ...
    const inputField = {
        value: `{"items":[{"offer":{"merchantId":"${merchant}"},"goods":{"goodsId":"${goodId}"},"quantity":1}],"cartInfo":{"type":"CART_TYPE_DEFAULT","locationId":""}}`
    };
...
async function parseGoodsContinuously(goodId, merchantId) {
    ...
    while (true) {
        ...
        await new Promise(resolve => setTimeout(resolve, 1000)); // Например, задержка в 10 секунд
    }
